  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: (res) => {
      if (res) {
        ...
        queryClient.invalidateQueries({ queryKey: ['userOrders', userData?.$id] });
      } else {
  ...
  return (
    <>
    {
      ...
      <div className="flex items-start gap-6 text-sm">
        <img className="w-16 sm:w-20 object-cover rounded" src={getProductImage(order.image)} alt={order.title} />
        <div>
          ...
          <div className="flex items-center gap-3 mt-1 text-base text-gray-700">
            ...
            <p className="mt-1">Date: <span className=" text-gray-400">{new Date(order.$createdAt).toLocaleDateString()}</span></p>
